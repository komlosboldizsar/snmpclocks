...
namespace BToolbox.GUI.Tables
{
    public class CustomDataGridViewRow<T>: DataGridViewRow
    {
        ...
        {
            if (item is INotifyPropertyChanged itemCastedINotifyPropertyChanged)
        ...
        {
            if (item is not INotifyPropertyChanged itemCastedINotifyPropertyChanged)
                return;
            for (int i = 0; i < cells.Count; i++)
            {
                ...
                foreach (string[] eventNames in columnDescriptor.MultilevelChangeEvents)
                {
                    ...
                    MultilevelPropertyChangeObserver multilevelObserver = new(itemCastedINotifyPropertyChanged, eventNames, tagData);
        ...
        private void notifyMultilevelPropertyChangedHandler(string fullPropertyName, MultilevelPropertyChangeObserver observer)
        {
            ...
            if ((tagData == null) || (tagData.Length != 2))
                return;
            if (tagData[0] is not DataGridViewCell cell)
                return;
        ...
        public void HandleContentClick(DataGridViewCellEventArgs eventArgs)
        {
            ...
            DataGridViewCell cell = Cells[ci];
            if ((cell is DataGridViewDisableButtonCell disableButtonCell) && !disableButtonCell.Enabled)
                return;
            getColumnDescriptor(ci).ContentClickHandlerMethod?.Invoke(item, cell, eventArgs);
        }

        public void HandleDoubleClick(DataGridViewCellEventArgs eventArgs)
        {
            int ci = eventArgs.ColumnIndex; DataGridViewCell cell = Cells[ci];
            if ((cell is DataGridViewDisableButtonCell disableButtonCell) && !disableButtonCell.Enabled)
                return;
            getColumnDescriptor(ci).DoubleClickHandlerMethod?.Invoke(item, cell, eventArgs);
        }
